...
from ..core.say import _say_error

from ..core import easy_cozmo
from ..core.movements import *
from ..core.movements import _execute_go_to_pose
import numpy as np

from ..core.defaults import df_scan_cube_speed,\
...
def _cube_one():
        return _get_visible_cube_by_id(1)

def _cube_two():
        return _get_visible_cube_by_id(2)

def _cube_three():
        return _get_visible_cube_by_id(3)
...
aligned = False
align_failed = False
def _render_scan_text(image, message, color):
        d = ImageDraw.Draw(image)
        bounds = (0, 0, image.width, image.height)
        arialfont = ImageFont.truetype("arial.ttf", 28, encoding="unic")
        text = cozmo.annotate.ImageText(message, font=arialfont,
            position=cozmo.annotate.TOP_RIGHT, color=color)
...
def scan_anno(image, scale, annotator=None, world=None, **kw):
        for active, message, color in [
            (scanning, f'Scanning for cube {scan_id}', 'yellow'),
            (found, f'Found cube {scan_id}', 'green'),
            (scan_failed, 'Cube scan failed', 'red'),
            (calculating_dist, f'Calculating distance to cube {scan_id}', 'yellow'),
            (found_dist, f'Found distance to cube {scan_id}', 'green'),
            (dist_failed, 'Distance calculation failed', 'red'),
            (aligning, f'Aligning with cube {scan_id}', 'yellow'),
            (aligned, f'Aligned to cube {scan_id}', 'green'),
            (align_failed, 'Alignment with cube failed', 'red'),
        ]:
                if active:
                        _render_scan_text(image, message, color)
