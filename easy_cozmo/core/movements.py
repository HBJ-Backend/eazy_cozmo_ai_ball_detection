...
def _drive_straight(distance, speed, what):
    ...
    try:
        ...
        if action.has_succeeded:
            ...
            code, reason = action.failure_reason
            result = action.result
            print("WARNING: Move %s: code=%s reason='%s' result=%s" % (what, code, reason, result))

    except Exception as e:
        import traceback
        traceback.print_exc()
        print(e)
        say_error("I can't move %s" % what)
    try:
        while action.is_running:
            action.abort()
            time.sleep(.5)
...
def reverse(distance):
    ...
    return _drive_straight(-distance, df_reverse_speed, "backward")
...
def move_forward(distance=None):
    if distance is None:
        return move()
    return _drive_straight(distance, df_forward_speed, "forward")
