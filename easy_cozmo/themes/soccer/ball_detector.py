...
from ...core.movements import _move_head, move_lift_ground,_execute_go_to_pose, set_wheels_speeds, stop, move_head_looking_forward
...
from ...core.robot_utils import pause
from ...core.robot_utils import enable_head_light, disable_head_light
from ...core.say import *
import time
from ...core.odometry import *
from ...actions.actions_with_cubes import scan_for_cube_by_id, _get_visible_cube_by_id, _get_relative_pose, _get_localized_cube_by_id,distance_to_cube
from ...actions.actions_with_custom_markers import scan_for_marker_by_id, _get_visible_marker_by_id, center_marker, _get_localized_marker_by_id, distance_to_marker
_ball_detector = None
...
def _render_status_text(d, bounds, message, color, position=5):
    """Draw a status label on the annotated camera frame.

    Uses arial.ttf if available, otherwise the default font. position=None
    leaves ImageText to place the text itself.
    """
    extra = {} if position is None else {'position': position}
    text = None
    try:
        arialfont = ImageFont.truetype("arial.ttf", 28, encoding="unic")
        if arialfont is not None:
            text = cozmo.annotate.ImageText(message, font=arialfont, color=color, **extra)
    except:
        pass
    if text is None:
        text = cozmo.annotate.ImageText(message, color=color, **extra)
...
class BallAnnotator(cozmo.annotate.Annotator):
    ...
    def apply(self, image, scale):
        ...
        if distance is not None:
            ...
            text.render(d, bounds)
        det = self._detector
        # (active?, message, color) for each transient status overlay
        for active, message, color in [
            (det.scanning, 'Scanning for ball', 'yellow'),
            (det.found_ball, 'Ball Detected', 'green'),
            (det.ball_scan_failed, 'No ball found', 'red'),
            (det.aligning_with_ball, 'Aligning with ball', 'yellow'),
            (det.aligned_with_ball, 'Aligned with ball', 'green'),
            (det.ball_align_failed, 'Ball alignment failed', 'red'),
            (det.aligning_with_ball_and_cube, f'Aligning with ball and cube {det.align_ball_and_cube_id}', 'yellow'),
            (det.aligned_with_ball_and_cube, f'Aligned with ball and cube {det.align_ball_and_cube_id}', 'green'),
            (det.ball_cube_align_failed, 'Alignment with ball and cube failed', 'red'),
            (det.kicking, 'Kicking Ball', 'yellow'),
            (det.kicked, 'Ball Kicked', 'green'),
            (det.kick_failed, 'Kick failed', 'red'),
            (det.scanning_left_post, 'Scanning for left post', 'yellow'),
            (det.found_left_post, 'Found left post', 'green'),
            (det.left_post_scan_failed, 'No left post found', 'red'),
            (det.scanning_right_post, 'Scanning for right post', 'yellow'),
            (det.found_right_post, 'Found right post', 'green'),
            (det.right_post_scan_failed, 'No right post found', 'red'),
            (det.getting_ball_dist, 'Calculating Ball distance', 'yellow'),
            (det.got_ball_dist, 'Ball distance found', 'green'),
            (det.ball_dist_failed, 'Ball distance calculation failed', 'red'),
        ]:
            if active:
                _render_status_text(d, bounds, message, color)
...
class BallDetectionClient:
    def __init__(self, host='127.0.0.1', port=44444):
        self.host = host
        self.port = port
...
def init_ball_detection():
    ...
    if _ball_detection_initialized:
        return True
    from ...core.odometry import initialize_odometry
    ...
    if scan_for_ball(360, annotate = False):
        ...
        if align_with_ball2():
            ...
            if cube is not None:
                ...
                fix_virtual_ball_in_world(ball_pose, 30)
